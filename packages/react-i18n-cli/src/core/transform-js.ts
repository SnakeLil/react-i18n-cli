...
import { isObject } from "../utils/isObj";
import { IGNORE_REMARK } from "../constant";
...
function getObjectExpression(obj: TemplateParams): ObjectExpression {
  const ObjectPropertyArr: Array<
    ObjectMethod | ObjectProperty | SpreadElement
  > = [];
...
function isPropNode(path: NodePath<StringLiteral>): boolean {
  const objWithProps =
    path.parentPath?.parentPath?.parentPath?.parentPath?.parent;
  const rootNode =
    ...
    function getTraverseOptions() {
      return {
        ...
        TemplateLiteral(path: NodePath<TemplateLiteral>) {
          ...
          templateMembers.sort(
            (a, b) => (a.start as number) - (b.start as number)
          );

          const shouldReplace = node.quasis.some((node) =>
            includeChinese(node.value.raw)
          );
        ...
        JSXText(path: NodePath<JSXText>) {
          ...
          if (includeChinese(value)) {
            ...
            path.replaceWith(
              t.jSXExpressionContainer(getReplaceValue(translationKey))
            );
          }
          path.skip();
        },
        ...
        ObjectProperty(path: NodePath<ObjectProperty>) {
          if (t.isStringLiteral(path.node.key)) {
            if (includeChinese(path.node.key.value)) {
              hasTransformed = true;
              const translationKey = Collector.add(
                path.node.key.value,
                customizeKey
              );
              path.replaceWith(
